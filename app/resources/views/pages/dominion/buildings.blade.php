                      ...
                      <table class="table">
                          ...
                          @foreach ($buildingHelper->getBuildingsByRace($selectedDominion->race) as $building)
                              <tr class="text-normal">
                                  ...
                                  <td>
                                      <input type="number" name="{{ $building->key }}" class="form-control text-center" placeholder="0" min="0" max="{{ $constructionCalculator->getMaxAfford($selectedDominion) }}" value="{{ old('construct.' . $building->key) }}" {{ $selectedDominion->isLocked() ? 'disabled' : null }}>
                                  </td>
                                  <td>
