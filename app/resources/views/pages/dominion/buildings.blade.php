                          ...
                          @foreach ($buildings as $building)
                              @if(count(array_diff($building->excluded_races, [$selectedDominion->race->name])) !== 0 or count(array_diff($building->exclusive_races, [$selectedDominion->race->name])) == 0)
                                  <tr class="text-normal">
                                      ...
                                      <td>
                                          <input type="number" name="construct[building_{{ $building->key }}]" class="form-control text-center" placeholder="0" min="0" max="{{ $constructionCalculator->getMaxAfford($selectedDominion) }}" value="{{ old('construct.' . $buildingType) }}" {{ $selectedDominion->isLocked() ? 'disabled' : null }}>
                                      </td>
                                      <td>
